...
class UserService
{
    ...
    {
        return User::when($query, fn($q) => $q->where('name', 'LIKE', "%$query%")
    ...
    {
        return User::with(['images', 'privacySetting', 'chats' => fn($q) =>
            $q->where('type', 'private')
              ->whereHas('users', fn($q2) => $q2->where('users.id', $authUserId))
        ])->where('tag', $identifier)->firstOrFail();
    }
